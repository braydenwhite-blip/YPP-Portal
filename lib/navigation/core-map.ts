import type { NavRole } from "@/lib/navigation/types";

export const CORE_NAV_LIMIT = 8;

export const PRIMARY_ROLE_FALLBACK_ORDER: NavRole[] = [
  "ADMIN",
  "HIRING_CHAIR",
  "CHAPTER_PRESIDENT",
  "INSTRUCTOR",
  "MENTOR",
  "PARENT",
  "STUDENT",
  "STAFF",
  "APPLICANT",
];

export const CORE_NAV_MAP: Record<NavRole, string[]> = {
  APPLICANT: [
    "/",
    "/application-status",
    "/instructor-training",
    "/instructor/lesson-design-studio",
    "/positions",
    "/applications",
  ],
  STUDENT: [
    "/",
    "/my-chapter",
    "/curriculum",
    "/my-program",
    "/pathways",
    "/my-classes",
    "/events",
  ],
  INSTRUCTOR: [
    "/",
    "/instructor/classes",
    "/instructor/schedule",
    // Kept for core-map floor / full-explorer only — filtered out of the
    // default instructor sidebar by INSTRUCTOR_V1_ALLOWED_HREFS.
    "/instructor/workspace",
    "/instructor/class-settings",
  ],
  ADMIN: ["/", "/mentorship", "/actions", "/admin/applicants", "/admin/users", "/admin/chapters"],
  HIRING_CHAIR: ["/", "/people", "/actions", "/admin/instructor-applicants"],
  PARENT: ["/", "/parent", "/parent/resources", "/calendar", "/parent/connect", "/goals"],
  CHAPTER_PRESIDENT: [
    "/",
    "/mentorship",
    "/actions",
    "/chapter-lead/instructor-applicants",
    "/chapter",
    "/partners",
  ],
  MENTOR: ["/", "/mentorship", "/mentorship/mentees", "/mentorship/feedback", "/office-hours"],
  STAFF: ["/", "/mentorship", "/actions", "/admin/instructor-applicants", "/leadership-pathway", "/admin/chapters"],
};
