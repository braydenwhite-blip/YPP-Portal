...
export async function saveOnboardingProfile(formData: FormData) {
  ...
  const school = formData.has("school")
    ? parseOptionalSchool((formData.get("school") as string | null) ?? "")
    : undefined;
  const grade = formData.has("grade")
    ? parseOptionalStudentGrade((formData.get("grade") as string | null) ?? "")
    : undefined;
  const interests = formData.has("interests")
    ? parseStudentInterests(formData.getAll("interests").map(String))
    : undefined;
  const learningStyle = formData.has("learningStyle")
    ? parseStudentLearningStyle(formData.get("learningStyle") as string | null)
    : undefined;
  const primaryGoal = formData.has("primaryGoal")
    ? parseStudentPrimaryGoal(formData.get("primaryGoal") as string | null)
    : undefined;
  const parentEmail = formData.has("parentEmail")
    ? parseOptionalEmail((formData.get("parentEmail") as string | null) ?? "", "parent email")
    : undefined;
