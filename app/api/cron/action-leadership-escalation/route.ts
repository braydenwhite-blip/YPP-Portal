...
/**
 * Leadership Escalation + Board roll-up cron — People Strategy Action Tracker.
 *
 * Schedule (UTC): `0 7 * * *` (daily, morning). Two stages:
 *   1. Leadership escalation — flagged/OVERDUE items unresolved 48h+ are escalated to
 *      the Leadership exactly once (`escalatedToLeadershipAt`).
 *   2. Board roll-up — Leadership-escalated items still unresolved 7 days past the Leadership
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const escalation = await runLeadershipEscalations(now);
    const boardRollup = await runBoardRollups(now);
    return NextResponse.json({ ok: true, escalation, boardRollup });
  } catch (err) {
    logger.error({ err }, "action-leadership-escalation cron failed");
