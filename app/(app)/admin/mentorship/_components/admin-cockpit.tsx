...
import { ReviewCyclesSection } from "@/components/mentorship/review-cycles-section";
...
/**
 * The admin mentorship cockpit — the full eight-tab oversight surface, now
 * mounted inside the unified hub at /mentorship?view=admin (the old
 * /admin/mentorship page redirects there). The hub owns the PageHeaderV2 and
 * the POV gate: `hasMentorshipCommandAccess` implies the ADMIN role (via
 * isLeadershipOrBoard), so everyone who reaches this component could open the
 * old ADMIN-gated cockpit.
 */

// Every internal cockpit link re-enters the hub on the admin POV.
const COCKPIT_BASE = "/mentorship?view=admin";
...
export type AdminCockpitSearchParams = {
  tab?: string;
  lane?: string;
  who?: string;
  menteeId?: string;
...
// `?lane=` is shared: development-lane values (overview lifecycle filter) and
// admin mentorship lanes (assignments tab) never collide, so each tab parses
// the param with its own vocabulary and ignores the other's values.
function parseDevelopmentLane(value?: string): DevelopmentLaneId | null {
...
export async function AdminMentorshipCockpit({
  searchParams,
  showLeadershipOverview,
}: {
  searchParams: AdminCockpitSearchParams;
  /**
   * Leadership command-center gate (`hasMentorshipCommandAccess`). The review
   * cycles + lifecycle lanes on the overview tab were leadership-only before
   * the merge; a plain ADMIN keeps the rest of the cockpit without them.
   */
  showLeadershipOverview: boolean;
}) {
  ...
  const developmentLaneFilter = parseDevelopmentLane(searchParams.lane);
  ...
  const [data, goalReviews, monthlyReviews, opsSummary, commandCenter] =
    await Promise.all([
      ...
      getInstructorMentorshipOpsSummary(),
      // Review cycles + lifecycle lanes, merged into the overview tab from the
      // hub's old thin command-center view.
      tab === "overview" && showLeadershipOverview
        ? loadMentorshipCommandCenter(who === "officers" ? "officer" : "instructor")
  ...
  return (
    <div className="mx-auto flex w-full max-w-[1440px] flex-col gap-6">
      ...
      <TrackerStartCard
        ...
        action={
          <ButtonLink href={`${COCKPIT_BASE}&tab=needs-attention`} variant="secondary" size="sm">
            Review alerts
          </ButtonLink>
        }
        facts={[
          {
            ...
            href: `${COCKPIT_BASE}&tab=assignments`,
          },
          {
            label: "unstaffed",
          ...
          },
          {
            ...
            href: `${COCKPIT_BASE}&tab=needs-attention`,
          ...
          },
          {
            ...
            href: `${COCKPIT_BASE}&tab=approvals`,
          },
        ]}
      />
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        {isMentorship2Enabled() && (
          <ButtonLink href="/admin/mentorship/applications" variant="secondary" size="sm">
            Applications
          </ButtonLink>
        )}
      </div>
      ...
      {tab === "overview" && (
        <div className="flex flex-col gap-6">
          {/* Leadership-only blocks (commandCenter loads only with access). */}
          {commandCenter ? (
            <>
              <ReviewCyclesSection cycles={commandCenter.activeCycles} />

              <DevelopmentCockpit
                overview={commandCenter.overview}
                who={who}
                laneFilter={developmentLaneFilter}
              />

              {commandCenter.mentorCoverage.needsMentorCount > 0 ? (
                // Mentor load & coverage details live on the Workload tab —
                // one line here instead of a duplicate table.
                <p className="m-0 px-1 text-[12.5px] text-ink-muted">
                  {commandCenter.mentorCoverage.needsMentorCount} without a mentor ·{" "}
                  {commandCenter.mentorCoverage.coveredCount}/
                  {commandCenter.mentorCoverage.mentorEligibleCount} covered — see{" "}
                  <Link
                    href={`${COCKPIT_BASE}&tab=capacity`}
                    className="font-semibold text-brand-700 hover:underline"
                  >
                    Workload
                  </Link>{" "}
                  for mentor load &amp; coverage.
                </p>
              ) : null}
            </>
          ) : null}
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {ADMIN_MENTORSHIP_LANES.map((laneOption) => {
              ...
              return (
                <Link
                  key={laneOption}
                  href={`${COCKPIT_BASE}&tab=assignments&lane=${toLaneQueryValue(laneOption)}`}
                  scroll={false}
