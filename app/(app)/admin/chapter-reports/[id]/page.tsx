...
export default async function AdminChapterReportDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="mx-auto w-full max-w-5xl px-6 py-8"><PageHeaderV2 eyebrow={`${report.chapter.name} · ${report.type} · ${report.status}`} title={formatReportingPeriod(report.periodStart, report.periodEnd)} subtitle={`Snapshot saved by ${report.createdBy.name} on ${report.updatedAt.toLocaleString()}.`} actions={<ButtonLink href="/admin/chapter-reports" variant="secondary" size="sm">All chapter reports</ButtonLink>} />
    ...
    <section className="mt-8"><h2 className="text-base font-semibold">Source records</h2><p className="mt-1 text-sm text-ink-muted">The exact records captured when this snapshot was saved.</p><div className="mt-3 divide-y divide-slate-100 border-y border-slate-200">{Object.entries(metricRecords).map(([key, raw]) => { const refs = refsFrom(raw); return <details key={key} className="px-3 py-2 text-sm"><summary className="flex cursor-pointer list-none items-center justify-between gap-4"><span className="font-medium text-slate-700">{label(key)}</span><span className="text-ink-muted">{refs.length} record{refs.length === 1 ? "" : "s"}{refs.length ? " · View" : ""}</span></summary>{refs.length ? <div className="mt-2 max-h-48 space-y-1 overflow-y-auto border-l border-slate-200 pl-3">{refs.map((ref, index) => <div key={`${ref.id}-${index}`}><Link href={ref.href} className="font-medium text-brand-700">Open record {index + 1}</Link><span className="ml-2 font-mono text-xs text-ink-muted">{ref.id}</span></div>)}</div> : <p className="mt-2 text-xs text-ink-muted">No source records contributed to this metric.</p>}</details>; })}</div></section>
