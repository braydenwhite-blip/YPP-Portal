...
import { getAnnouncementsForUser } from "@/lib/announcement-actions";
...
export default async function StudentHome() {
  ...
  const announcements = await getAnnouncementsForUser(user.id);
      ...
      <S8Grid>
        {announcements.length ? (
          <S8Card title="Announcements">
            <S8List items={announcements} empty="" render={(a: any) => <S8Item key={a.id} title={a.title} meta={shortDate(a.publishedAt)}>{a.content}</S8Item>} />
          </S8Card>
        ) : null}
