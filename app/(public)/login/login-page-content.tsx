...
import { SUPABASE_PUBLIC_ENV_MISSING_MESSAGE } from "@/lib/supabase/config";
import { isLegacyAuthBypassEmail } from "@/lib/legacy-auth-config";

/**
 * Auth setup flags must come from the server. Reading non-NEXT_PUBLIC_ env (or
 * creating a browser Supabase client during SSR) can disagree with the client
 * and cause a hydration mismatch on /login.
 */
export function LoginPageContent({
  localPasswordFallbackEnabled,
  hasSupabasePublicAuth,
}: {
  localPasswordFallbackEnabled: boolean;
  ...
  const hasSupabaseBrowserAuth = hasSupabasePublicAuth;
  // Create only for submit handlers — do not use for render branching (hydration).
