...
import { submitTechnologyManagerApplication } from "@/lib/application-actions";
import {
  getFieldError,
...
const STORAGE_KEY = "ypp-technology-manager-draft";
...
const STEPS = [
  ...
  },
  {
    id: "experience",
    title: "Experience",
    description: "Tools, experience, and portfolio",
...
export default function TechnologyManagerApplicationForm({
  disabled = false,
}: {
  disabled?: boolean;
}) {
  ...
  async function handleSubmit() {
    ...
    setSubmitting(true);
    try {
      const result = await submitTechnologyManagerApplication(formData);
    ...
    return (
      <div style={{ padding: 24, background: "#f0fdf4", borderRadius: 12, textAlign: "center" }}>
        ...
        <p style={{ color: "var(--muted)", margin: "0 0 16px" }}>
          Thanks for applying to be a Technology Manager. You&apos;ll get email updates as your
  ...
  return (
    <div>
      ...
      {currentStep === 1 ? (
        <div style={{ display: "grid", gap: 14 }}>
          <label style={{ display: "grid", gap: 4 }}>
            <span style={{ fontWeight: 600, fontSize: 13 }}>Technical interests / tools</span>
            <FieldHint text="e.g. portal tools, automations, Google Workspace, coding, troubleshooting" />
            <input
              className="input"
              value={fields.platforms}
              disabled={disabled}
              onChange={(e) => setField("platforms", e.target.value)}
              placeholder="Portal tools, automations, …"
            />
          ...
          <label style={{ display: "grid", gap: 4 }}>
            ...
            <FieldHint text="No prior experience required — tell us what you know about building, fixing, or supporting tech." />
            <textarea
              className="input"
              rows={5}
              ...
              placeholder="Club websites, helping teachers with tech, scripts, docs, troubleshooting…"
            />
            <FieldError message={getFieldError(errors, "experience")} />
          </label>
          <label style={{ display: "grid", gap: 4 }}>
            <span style={{ fontWeight: 600, fontSize: 13 }}>Portfolio / project links (optional)</span>
            <FieldHint text="Paste links to GitHub, sites, docs, or anything that shows your work." />
            <textarea
              className="input"
              rows={3}
      ...
      {currentStep === 2 ? (
        <div style={{ display: "grid", gap: 14 }}>
          <label style={{ display: "grid", gap: 4 }}>
            ...
            <FieldHint text="What draws you to YPP’s tech team and youth empowerment?" />
            <textarea
              className="input"
              rows={5}
          ...
          <label style={{ display: "grid", gap: 4 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
              <span style={{ fontWeight: 600, fontSize: 13 }}>Ideas for YPP systems</span>
              <CharCount value={fields.contentIdeas} max={3000} />
            </div>
            <FieldHint text="One or more ideas for improving portal tools, onboarding, admin workflows, or chapter support." />
            <textarea
              className="input"
              rows={4}
