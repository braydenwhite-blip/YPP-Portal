...
/** Sentinel value in the mentor picker for Board member opt-out. */
export const BOARD_MEMBER_SENTINEL = "__BOARD_MEMBER__";
...
> = {
  mentor: {
    ...
    change: "Change",
    pickError: "Pick a mentor or Board member.",
    help: "Pick a mentor, or mark Board member if they do not need one.",
  },
  chair: {
    noun: "Role Chair",
 ...
 * without leaving the page. Mentor picker includes a Board member opt-out.
 */
export function PeopleMentorAssignCell({
  ...
  /** Mentor kind only — Board member / no mentee mentor needed. */
  mentorshipExempt?: boolean;
}) {
  const router = useRouter();
  const copy = COPY[kind];
  const displayName =
    kind === "mentor" && mentorshipExempt
      ? "Board member"
      : mentorName;
  const [open, setOpen] = useState(false);
  const [selected, setSelected] = useState(
    mentorshipExempt && kind === "mentor" ? BOARD_MEMBER_SENTINEL : mentorId ?? "",
  );
  ...
  function openModal(event: MouseEvent) {
    ...
    setSelected(
      mentorshipExempt && kind === "mentor"
        ? BOARD_MEMBER_SENTINEL
        : mentorId ?? "",
    );
    setQuery("");
    setError(null);
  ...
  function save() {
    ...
    startTransition(async () => {
      try {
        ...
        } else if (selected === BOARD_MEMBER_SENTINEL) {
          await setMentorshipExempt({
            userId: menteeId,
            exempt: true,
            reason: "Marked Board member from People dashboard",
          });
        } else {
      ...
      <div className="flex min-w-0 flex-col gap-1" onClick={(e) => e.stopPropagation()}>
        ...
        {canAssign ? (
          blockedByMissingMentorship ? (
            ...
            >
              {displayName ? copy.change : copy.assign}
            </button>
          )
        ) : null}
        {error && !open ? (
      ...
      >
        <div className="flex flex-col gap-3" onClick={(e) => e.stopPropagation()}>
          <div>
            ...
            >
              {displayName ? `Change ${copy.noun}` : `Assign ${copy.noun}`}
            </h2>
              ...
              <li>
                <label
                  className={cn(
                    ...
                    selected === BOARD_MEMBER_SENTINEL && "border-brand-300 bg-brand-50/50",
                  )}
                >
                  <input
                    type="radio"
                    ...
                    checked={selected === BOARD_MEMBER_SENTINEL}
                    onChange={() => setSelected(BOARD_MEMBER_SENTINEL)}
                  />
