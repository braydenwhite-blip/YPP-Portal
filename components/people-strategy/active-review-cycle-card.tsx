...
const STAGE_LABELS: Record<string, string> = {
  ...
  REVIEW_SUBMITTED: "Waiting on chair approval",
...
export function ActiveReviewCycleCard({ cycleState }: { cycleState: CycleState }) {
  ...
  return (
    <CardV2 className="border-l-[3px] border-l-brand-600">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        {cycleState.nextAction.href && cycleState.nextAction.key !== "all-caught-up" ? (
      ...
      <div className="mt-3 flex flex-wrap items-center gap-4 text-[13px] text-[#4a4a5e]">
        ...
        <span>
          ...
          {OWNER_LABELS[cycleState.nextAction.ownerRole] ?? cycleState.nextAction.ownerRole}
          {cycleState.nextAction.ownerName ? ` (${cycleState.nextAction.ownerName})` : ""}
        </span>
