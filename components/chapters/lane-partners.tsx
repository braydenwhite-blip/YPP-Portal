...
export function LanePartners({ chapterId, view, operations }: { chapterId: string; view: ChapterLaneView; operations?: PartnerOperations }) {
  return (
    <div className="flex flex-col gap-6">
      ...
      {operations && <div className="grid gap-5 lg:grid-cols-2"><section><h3 className="m-0 text-[13.5px] font-bold text-ink">Partner pipeline</h3><p className="m-0 text-[12px] text-ink-muted">Current relationship stage from each partner record.</p><div className="mt-3 flex flex-wrap border-y border-slate-200">{operations.pipeline.map((row) => <a key={row.stage} href={row.href} className="min-w-[112px] flex-1 px-3 py-3 no-underline hover:bg-slate-50"><p className="text-[11px] uppercase tracking-wide text-ink-muted">{row.label}</p><p className="mt-1 text-xl font-semibold tabular-nums text-slate-950">{row.count}</p></a>)}</div></section><section><h3 className="m-0 text-[13.5px] font-bold text-ink">Agreement pipeline</h3><p className="m-0 text-[12px] text-ink-muted">Derived from the agreements already attached to partner records.</p><div className="mt-3 flex flex-wrap border-y border-slate-200">{operations.agreementStages.map((row) => <a key={row.stage} href="/partners" className="min-w-[112px] flex-1 px-3 py-3 no-underline hover:bg-slate-50"><p className="text-[11px] uppercase tracking-wide text-ink-muted">{row.label}</p><p className="mt-1 text-xl font-semibold tabular-nums text-slate-950">{row.count}</p></a>)}</div></section></div>}
