...
export default function ApplicantNextActionBar({
  ...
  const isWaiting = !action.handler && !action.href;

  // Inline "next required move" hero — lives at the TOP of the review canvas, not
  // a fixed bar that covers content. It names the single most important step and
  // gives one clear way forward.
  return (
    <section
      aria-label="Next required move"
      className={`rounded-[14px] border p-5 shadow-card ${
        isWaiting
          ? "border-line-soft bg-surface"
          : "border-brand-200 bg-gradient-to-br from-brand-50 to-surface"
      }`}
    >
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="flex min-w-0 items-start gap-3">
          <span
            ...
            className={`mt-0.5 flex size-9 shrink-0 items-center justify-center rounded-[10px] ${
              isWaiting ? "bg-surface-soft text-ink-muted" : "bg-brand-600 text-white"
            }`}
          >
          ...
          <div className="min-w-0">
            <p className="m-0 text-[11px] font-bold uppercase tracking-[0.11em] text-brand-700">
              {isWaiting ? "Current status" : "Next required move"}
            </p>
            <p className="m-0 text-[18px] font-bold leading-tight text-ink">{action.label}</p>
            <p className="m-0 mt-0.5 text-[13px] leading-relaxed text-ink-muted">
              {action.description}
            </p>
            {message && (
              <p
                className={
                  message.ok
                    ? "m-0 mt-1 text-[12.5px] font-semibold text-success-700"
                    : "m-0 mt-1 text-[12.5px] font-semibold text-danger-700"
                }
              >
                {message.text}
              </p>
        ...
        <div className="shrink-0">
          {action.handler ? (
            <button
              type="button"
              className="inline-flex cursor-pointer items-center justify-center rounded-[8px] bg-brand-600 px-4 py-2.5 text-[13.5px] font-semibold text-white shadow-card hover:bg-brand-700 disabled:pointer-events-none disabled:opacity-50"
            ...
            <a
              href={action.href}
              className="inline-flex items-center justify-center rounded-[8px] bg-brand-600 px-4 py-2.5 text-[13.5px] font-semibold text-white shadow-card hover:bg-brand-700"
            >
              {action.label}
            </a>
          ) : (
