...
const WORKSPACE_GUIDE_ITEMS = [
  ...
  },
  {
    ...
    meaning:
      "Next steps are the concrete follow-ups that come out of a check-in.",
    howToUse:
      "Write one clear next step at a time, assign an owner when possible, and add a due date if you want the system to help track follow-through.",
  },
  {
...
export async function RelationshipWorkspace({
  menteeId,
  backHref = "/mentorship/mentees",
  backLabel = "Your mentees",
  badge = null,
  subtitle = "Full relationship workspace: people, check-ins, next steps, requests, resources, and progress signals.",
  adminControls = null,
  ...
  if (operationsEnabled) {
    const [classes, rdy] = await Promise.all([
  ...
  return (
    <div>
      ...
      {adminControls ? (
        <div style={{ marginBottom: 16 }}>{adminControls}</div>
      ) : null}
