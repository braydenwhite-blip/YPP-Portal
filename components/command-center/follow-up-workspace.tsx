...
import { cn, StatusBadge } from "@/components/ui-v2";
import type {
  CcComposerTarget,
  CcFollowUpItem,
  CcFollowUpType,
...
import { EmptySimpleState, SimpleActionStrip, SimpleListCard, SimpleRow, type SimpleAction } from "./simple";
...
const FOLLOW_UP_ACTIONS: SimpleAction[] = [
  { label: "Send reminder", href: "/messages", icon: "send" },
  { label: "Create action", href: "/actions/new", icon: "check" },
  { label: "Add to meeting", href: "/meet", icon: "calendar" },
  { label: "Mark followed up", href: "/work/queue?queue=waiting", icon: "inbox" },
];

/** Calm: a single clean waiting-on list. Who · what they owe · related · status. */
function FollowUpCalm({ vm }: { vm: FollowUpWorkspaceVM }) {
  const rows = vm.items.slice(0, 6);
  return (
    <WorkspaceBody>
      {rows.length > 0 ? (
        <SimpleListCard>
          {rows.map((item) => (
            <SimpleRow
              key={item.id}
              href={item.href}
              name={item.personName ?? "Unassigned"}
              what={item.title}
              related={item.relatedLabel}
              status={{ label: item.stateLabel, tone: item.stateTone }}
              meta={item.dueLabel}
            />
          ))}
        </SimpleListCard>
      ) : (
        <EmptySimpleState>No one is blocking active work. You&apos;re all caught up.</EmptySimpleState>
      )}

      <SimpleActionStrip actions={FOLLOW_UP_ACTIONS} />

      <BrowseAllPanel label="Browse all follow-ups" hint="Every follow-up by person, type, and status.">
...
/** Executive: filters, waiting-people, stale, by-type, and the composer. */
function FollowUpExecutive({ vm }: { vm: FollowUpWorkspaceVM }) {
  ...
  return (
    <WorkspaceBody>
      <MissionBriefCard icon="inbox" eyebrow="Follow-ups" headline={vm.brief} />
      ...
      <div className="grid items-start gap-4 xl:grid-cols-[minmax(0,1fr)_320px]">
        <div className="flex flex-col gap-4">
          <PanelCard icon="hourglass" title="Waiting on people" action={<ViewAllLink href="/delegate">View owners</ViewAllLink>}>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <PanelCard icon="layers" title="By type">
              {vm.byType.length > 0 ? (
...
function FollowUpInner({ vm }: { vm: FollowUpWorkspaceVM }) {
  ...
  return (
    <WorkspaceShell className="px-1 pb-12">
      <WorkspaceHeader
        title="Follow Ups"
        lede="What's waiting on people."
        actions={<CommandModeToggle />}
      />
      {executive ? <FollowUpExecutive vm={vm} /> : <FollowUpCalm vm={vm} />}
    </WorkspaceShell>
  );
}
